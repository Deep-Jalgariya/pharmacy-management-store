...
class Main {
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("========>> Welcome to Ganesh MedicalStore Management System <<========");
        int choice, r;
    ...
    static void bill(double a) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(fileName))) {
            writer.write("************* GANESH MEDICAL STORE *****************\n");
            writer.write("\n-------------: Medicine Bill :-------------\n");
    
            for (Purchase p : pur) {
                double price = getPrice(p.getMedicineName());
                double total = price * p.getQuantity();
                writer.write("Medicine name: " + p.getMedicineName() + "\n");
            ...
            writer.write("\n-------------------------------------------\n");
            writer.write("Total amount: " + a + "\n");
            double totalWithGST = a + (a * 0.04);
            writer.write("Total amount with GST (4%): " + totalWithGST + "\n");
            writer.write("Thank you for shopping! Exiting customer operations.\n");
    
            System.out.println("Bill has been saved to " + fileName);
        } catch (IOException e) {
            System.out.println("An error occurred while saving the bill: " + e.getMessage());
        }
        ...
        for (Purchase p : pur) {
            double price = getPrice(p.getMedicineName());
            double total = price * p.getQuantity();
            System.out.println("Medicine name: " + p.getMedicineName() + "\n" + " Price: " + price + "\n" + " Quantity: "
                    + p.getQuantity() + "\n" + " Total: " + total);
